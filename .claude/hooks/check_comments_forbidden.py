...
def main() -> int:
    ...
    msg = (
        ...
        f"The datasheet can be e.g. a PDF. BSP source is a file name "
        f"with the BSP package name itself. A project document and "
        f"ANYTHING within the project itself (another .cpp file, etc) "
        f"is not a citation but a rotting comment. MSDN citation "
        f"Requires a LINK and this link has to be opened by agent itself "
        f"otherwise it is a fabricated tech spec and the logic you are doing "
        f"is based on fabrication (even if you delete the comment)\n"
