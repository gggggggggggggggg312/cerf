...
public:
    ...
    /* PIUAB0REG holds CMDScanDATA during CmdScan (VR4121 UM Table 20-5, VR4102 UM
       Table 19-5); PIUAB1-3 carry no command-scan data. */
    uint16_t ReadHalf2(uint32_t off) override {
        std::lock_guard<std::mutex> lk(mtx_);
        if (off == kOffAb0) return adbuf_;
        int page = 0, idx = 0;
...
private:
    ...
    void CmdScanOnceLocked() {
        ...
        adbuf_ = val ? static_cast<uint16_t>(kValid | (*val & 0x3FFu)) : 0u;
