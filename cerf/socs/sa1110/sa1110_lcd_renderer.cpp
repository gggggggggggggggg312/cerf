...
/* SA-1110 §11.7.1.5 Table 11-7: 4-bit palette dither value -> panel
   intensity, "0% Is Black"; values 14 and 15 are both always-on. */
constexpr uint8_t kMonoGray[16] = { 0, 28, 51, 68, 85, 102, 113, 128,
                                    142, 153, 170, 187, 204, 227, 255, 255 };
...
private:
    MonoLayout ResolveMonoLayout(const uint8_t* fb_base) {
        ...
        LOG(Lcd, "Sa1110LcdRenderer: FATAL mono frame with PBS=%u "
    ...
    void RenderMonoInto(uint32_t* dib_bgra32,
        ...
        if (!mono_palette_logged_) {
            ...
            LOG(Lcd, "Sa1110LcdRenderer: mono frame %ubpp, palette "
