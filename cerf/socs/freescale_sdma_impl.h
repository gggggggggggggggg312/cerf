...
class FreescaleSdmaBase : public Peripheral {
public:
...
private:
    ...
    void RefreshIrq() {
        /* An AP interrupt is sent only for channels whose INTRMASK bit is set
           (MCIMX31RM §40.10.3.11; i.MX51 SDMA is the same IP). */
        if ((intr_ & intrmask_) != 0) AssertIrqLine();
    ...
    void ResetCore() {
        ...
        for (uint32_t i = 0; i < kChannelCount; ++i) chnpri_[i] = 0;
        ResetExtra();
    }
