...
/* VR41xx AIU (Audio Interface Unit), 0x0B000160-0x0B00017F (VR4121 UM ch.21 Table 21-1
   p495 == VR4102 UM ch.20 Table 20-1). Casio wavedev.dll IST sub_1340AB4 (CreateThread'd by
   sub_1340EAC) blocks on the AIU interrupt event (SYSINTR 20, dword_13438D0) CERF never
   raises - off the boot thread, no boot deadlock. */
constexpr uint32_t kBase = 0x0B000160u;
constexpr uint32_t kSize = 0x20u;

constexpr uint32_t kOffScnt  = 0x08u;   /* SCNTREG  0x168 (VR4121 UM 21.2.4 p499 / VR4102 20.2.4 p413) */
constexpr uint32_t kOffScnvr = 0x0Au;   /* SCNVRREG 0x16A (VR4121 UM Table 21-1 / VR4102 20.2.5 p414) */
constexpr uint32_t kOffMcnt  = 0x12u;   /* MCNTREG  0x172 (VR4121 UM 21.2.7 p502 / VR4102 20.2.7 p416) */
constexpr uint32_t kOffSeq   = 0x1Au;   /* SEQREG   0x17A (VR4121 UM 21.2.10 p505 / VR4102 20.2.10 p419) */
constexpr uint32_t kOffInt   = 0x1Cu;   /* INTREG   0x17C (VR4121 UM 21.2.11 p506 / VR4102 20.2.11 p420) */

/* SEQREG: D15 AIURST, D4 AIUMEN, D0 AIUSEN R/W; D14:5 / D3:1 reserved (VR4121 p505 == VR4102 p419). */
...
/* SCNTREG: D15 DAENAIU + D1 SSTOPEN R/W; D3 SSTATE = AIUSEN speaker-run status (R),
   valid only when AIUSEN set; D14:4 / D2 / D0 reserved (VR4121 p499 == VR4102 p413). */
...
/* MCNTREG: D15 ADENAIU + D1 MSTOPEN R/W; D3 MSTATE = AIUMEN (R); D0 ADREQAIU (R,
   0 with no A/D request); D14:4 / D2 reserved (VR4121 p502 == VR4102 p416). */
...
/* INTREG: MIC {MENDINTR D11, MINTR D10, MIDLEINTR D9, MSTINTR D8} + SPEAKER
   ...
   written", VR4121 p506 == VR4102 p420). No source sets them while the unit is disabled. */
...
public:
    ...
    void OnReady() override {
        ...
        /* SCNTREG, MCNTREG, SEQREG and INTREG all have RTCRST and Other-resets rows of 0
